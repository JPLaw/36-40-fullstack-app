...
const USERNAME_ERROR = `Username must be ${MIN_USERNAME_LENGTH} characters long.`;
const EMAIL_ERROR = 'Invalid email';
const PASSWORD_ERROR = 'Invalid password';
const CONFLICT_ERROR = 'Username of email already exists';
  ...
  render() {
    ...
    return (
      <form className="auth-form" onSubmit={ this.handleSubmit }>
        {
          renderIf(this.state.usernameDirty, <h4 className="error">{ USERNAME_ERROR }</h4>)
        }
        <input 
          name="username"
        ...
        {
          renderIf(this.state.emailDirty, <h4 className="error">{ EMAIL_ERROR }</h4>)
        }
        <input 
          name="username"
          placeholder="username"
        ...
        {
          renderIf(this.state.passwordDirty, <h4 className="error">{ PASSWORD_ERROR }</h4>)
        }
        <input 
          name="password"
        ...
        />
        {
          renderIf(this.state.conflictError, <h4 className="error">{ CONFLICT_ERROR }</h4>)
        }
